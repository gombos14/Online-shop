...
export class BackendService {
  ...
  private invoke(method: string, url: string, body: any = {}, params?: any, responseType?: any): Observable<any> {
    if (!url) {
      throw new Error('No URL provided.');
    }
    const requestUrl = `${url}`;
    return this.http.request(method, requestUrl, {body, params, responseType});
  }
